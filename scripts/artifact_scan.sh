...
if git ls-files | grep -E -q '(^|/)(__pycache__|\.pytest_cache|node_modules|dist|target)/|\.(pyc|pyo|sqlite3?|dump|tar\.gz)$'; then
  echo "Generated artifacts are tracked by Git." >&2
  exit 1
fi
if git ls-files --others --exclude-standard | grep -E -q .; then
