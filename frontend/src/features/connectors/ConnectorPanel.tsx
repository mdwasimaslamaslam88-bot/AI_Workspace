...
export function ConnectorPanel({
  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    try {
      const [nextSettings, nextConnectors, nextAudit] = await Promise.all([
  ...
  }, [onLoad, onLoadAudit, onLoadSettings]);
    ...
    <section className="side-panel connector-panel" aria-labelledby="connector-heading">
      ...
      {settings?.configured !== true ? (
        ...
        <fieldset disabled={busy}>
          ...
          {authKind !== "none" && <label>Credential<input type="password" autoComplete="off" value={credential} minLength={16} maxLength={512} onChange={(event) => setCredential(event.target.value)} /></label>}
      ...
      <div className="connector-list">
        {connectors.map((connector) => (
          <article className="connector-card" key={connector.id}>
            ...
            <p className="field-help">{connector.kind.replaceAll("_", " ")} · {connector.base_url}</p>
